...
class CRUDSourceChecksum(
    CRUDBase[SourceChecksum, SourceChecksumCreate, SourceChecksumUpdate]
):
    def get_multi_filter_by_source(
        self, db: Session, *, skip: int = 0, limit: int = 100, source: str
    ) -> List[SourceChecksum]:
        return (
            db.query(self.model)
            .filter(self.model.source.ilike(f"%{source}%"))
            .offset(skip)
            .limit(limit)
            .all()
        )
