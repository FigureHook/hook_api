...
def test_get_multi_release_tickets(db: Session, client: TestClient):
    tickets = [create_random_release_ticket(db) for _ in range(random.randint(1, 10))]
    tickets_count = len(tickets)
    results_size = random.randint(1, 20)
    expected_pages = math.ceil(tickets_count / results_size) if tickets_count else 1
    ...
    response = client.get(
        url=v1_endpoint("/release-tickets"),
        params={"page": expected_page, "size": results_size, "purpose": "nothing"},
    )
    content = response.json()
    assert response.status_code == 200
...
def test_create_release_ticket(db: Session, client: TestClient):
    ...
    response = client.post(
        url=v1_endpoint("/release-tickets/"), json={"from": base_datetime.isoformat(), "purpose": "testing"}
    )
...
def test_get_release_ticket(db: Session, client: TestClient):
    ...
    response = client.get(
        url=v1_endpoint(f"/release-tickets/{ticket_uid.hex}"), params={"type": "feed"}
    )
    assert response.status_code == 200

    response = client.get(url=v1_endpoint(f"/release-tickets/{uuid.uuid4().hex}"))
    assert response.status_code == 404

    response = client.get(url=v1_endpoint("/release-tickets/123"))
    assert response.status_code == 404
...
def test_delete_release_ticket(db: Session, client: TestClient):
    ...
    assert type(ticket.id) is uuid.UUID
    ticket_uid = uuid.UUID(str(ticket.id))
    response = client.delete(url=v1_endpoint(f"/release-tickets/{ticket_uid.hex}"))
    assert response.status_code == 204

    response = client.get(url=v1_endpoint(f"/release-tickets/{ticket_uid.hex}"))
    assert response.status_code == 404
