...
rich_content = {
    "series": {"id": 0, "name": "name"},
    "category": {"id": 0, "name": "name"},
    "manufacturer": {"id": 0, "name": "name"},
    "releaser": {"id": 0, "name": "name"},
    "distributer": {"id": 0, "name": "name"},
    "sculptors": {"id": 0, "name": "name"},
    "paintworks": {"id": 0, "name": "name"},
    "official_images": {"id": 0, "order": 0, "url": "url"},
}
...
def test_get_products(client: TestClient, db: Session):
    ...
    expected_pages = ceil(products_count / results_size) if products_count else 1
...
def test_get_products_by_url(db: Session, client: TestClient):
    ...
    response = client.get(
        url=v1_endpoint("/products"), params={"source_url": choiced_product.url}
    )
    assert response.status_code == 200
...
def test_create_product(client: TestClient, db: Session):
    post_data = {
        ...
        "paintworks": ["color", "grey"],
        "official_images": ["/full/01.png", "/full/02.jpg"],
    }

    response = client.post(v1_endpoint("/products/"), json=post_data)
...
def test_get_product(client: TestClient, db: Session):
    ...
    response = client.get(v1_endpoint(f"/products/{product.id}"))
    fetched_product = response.json()
    assert response.status_code == 200
    for key in check_keys:
        assert key in fetched_product

    response = client.get(v1_endpoint("/products/5"))
    assert response.status_code == 404


def test_delete_product(db: Session, client: TestClient):
    product = create_random_product(db=db)
    response = client.delete(v1_endpoint(f"/products/{product.id}"))
    assert response.status_code == 204

    response = client.delete(v1_endpoint("/products/122"))
    assert response.status_code == 404
...
def test_update_product(db: Session, client: TestClient):
    ...
    response = client.put(v1_endpoint(f"/products/{product.id}"), json=update_data)
    assert response.status_code == 200
    ...
    response = client.put(v1_endpoint("/products/1235"), json=update_data)
    assert response.status_code == 404
...
def test_create_product_release(db: Session, client: TestClient):
    ...
    response = client.post(
        v1_endpoint(f"/products/{product.id}/release-infos"), json=release_data
    )
    assert response.status_code == 201
    ...
    response = client.post(
        v1_endpoint("/products/1325/release-infos"), json=release_data
    )
    assert response.status_code == 404
...
def test_get_product_images(db: Session, client: TestClient):
    product = create_random_product(db)
    response = client.get(v1_endpoint(f"/products/{product.id}/official-images"))
    assert response.status_code == 200
    ...
    response = client.get(v1_endpoint("/products/12351235/official-images"))
    assert response.status_code == 404
...
def test_get_product_release_info_feed(db: Session, client: TestClient):
    ...
    path = v1_endpoint(f"/products/{product.id}/release-infos/45654/feed")
    response = client.get(path)
    assert response.status_code == 404
