...
def test_get_paintworks(db: Session, client: TestClient):
    ...
    expected_pages = ceil(worker_count / results_size) if worker_count else 1
...
def test_update_paintwork(db: Session, client: TestClient):
    paintwork = create_random_paintwork(db)
    update_data = {"name": "Another master"}
    response = client.put(v1_endpoint(f"/paintworks/{paintwork.id}"), json=update_data)
    assert response.status_code == 200

    content = response.json()
    assert content.get("name") == update_data.get("name")

    response = client.put(v1_endpoint("/paintworks/12341234"), json=update_data)
    assert response.status_code == 404
