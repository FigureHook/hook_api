...
def check_category_exist(
    category_id: int, db: Session = Depends(deps.get_db)
) -> Category:
...
async def get_categories(
    *, db: Session = Depends(deps.get_db), page_params: PageParamsBase = Depends()
    ...
    categories_out = [CategoryInDB.from_orm(category) for category in categories]
    logger.info(f"Fetched the categories. (count={len(categories_out)})")
    return Page.create(
        results=categories_out, total_results=categories_count, params=page_params
    )


@router.post("/", response_model=CategoryInDB, status_code=status.HTTP_201_CREATED)
async def create_category(
    *, request: Request, db: Session = Depends(deps.get_db), category_in: CategoryCreate
):
