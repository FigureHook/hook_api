...
def check_paintwork_exist(
    paintwork_id: int, db: Session = Depends(deps.get_db)
) -> Paintwork:
...
async def get_paintwork(*, paintwork: Paintwork = Depends(check_paintwork_exist)):
...
async def update_paintwork(
    *,
    ...
    paintwork = crud.paintwork.update(db=db, db_obj=paintwork, obj_in=worker_in)
