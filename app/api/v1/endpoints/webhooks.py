...
def check_webhook_exist(channel_id: str, db: Session = Depends(deps.get_db)) -> Webhook:
    ...
    if not webhook:
        logger.info(f"Specified webhook didn't exist. (channel_id={channel_id})")
        raise HTTPException(
...
@router.get("/{channel_id}", response_model=DecryptedWebhookInDB)
async def get_webhook(
    *, webhook: Webhook = Depends(check_webhook_exist), channel_id: str
):
    logger.info(f"Fetched the webhook. (channel_id={channel_id})")
    return DecryptedWebhookInDB.from_orm(webhook)


@router.put("/{channel_id}", response_model=DecryptedWebhookInDB)
...
@router.patch("/{channel_id}", response_model=DecryptedWebhookInDB)
