...
def check_series_exist(series_id: str, db: Session = Depends(deps.get_db)) -> Series:
    ...
    if not series:
        logger.info(f"Specified series didn't exist. (id={series_id})")
        raise HTTPException(
...
@router.get("/", response_model=Page[SeriesInDB])
async def get_series_multi(
    *, db: Session = Depends(deps.get_db), page_params: PageParamsBase = Depends()
    ...
    series_out = [SeriesInDB.from_orm(series) for series in series_list]
    logger.info(f"Fetched the series. (count={len(series_out)})")
    return Page.create(
        results=series_out, total_results=series_count, params=page_params
    )
...
async def create_series(
    *,
    ...
    if series:
        logger.info(f"The series already exists. (id={series.id}, name={series.name})")
...
async def get_series(*, series: Series = Depends(check_series_exist)):
